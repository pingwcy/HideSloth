...
namespace HideSloth.Steganography
{
    public class SUNIWARD_Image : ImageAlgorithm
    {
        ...
        {
            {
                ...
                int[,,] imageData = new int[height, width, 4]; // 创建一个三维数组来存储RGBA数据
                ...
                for (int y = 0; y < height; y++)
                {
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        imageData[y, x, 0] = pixel.R;
                        imageData[y, x, 1] = pixel.G;
                        imageData[y, x, 2] = pixel.B;
                        imageData[y, x, 3] = pixel.A;
                    }
                }
                return imageData;
            }
        ...
        static int GetChannelCount(PixelFormat format)
        {
            ...
            switch (format)
            {
                ...
                case PixelFormat.Format32bppRgb:
                    return 4; // ARGB 或 RGBA
                              // 添加更多PixelFormat的case以处理其他情况
                default:
                    return 0; // 未知或不支持的格式
            }
        }
        ...
        public Bitmap Encode(Bitmap image, byte[] data, string pwd)
        {
            ...
            int m = 1;
            foreach (int i in new int[] { image.Width, image.Height, GetChannelCount(image.PixelFormat) })
            {
                m *= i;
            }
        ...
        public static (double[,], double[,]) CostFn(int[,,] I)
        {
            ...
            for (int i = 0; i < k; i++)
            {
                for (int j = 0; j < l; j++)
                {
                    ...
                    if (I[i, j, 0] == 255 && I[i, j, 1] == 255 && I[i, j, 2] == 255)
                    {
                        rhoP1[i, j] = INF;
                    }
                    if (rhoM1[i, j] > INF)
                    ...
                    if (I[i, j, 0] == 0 && I[i, j, 1] == 0 && I[i, j, 2] == 0)
                    {
                        rhoM1[i, j] = INF;
                    }
                }
